...
export function initAnalytics(key: string, host: string): void {
  ...
  // A visitor who withdrew consent earlier is still opted out in PostHog's own
  // persisted state, so accepting again has to opt them back in explicitly.
...
export function shutdownAnalytics(): void {
  ...
  // opt_out_capturing() is what actually stops capture and remembers the
  // choice. reset() alone only clears the identity — pageviews would keep
  // flowing after the visitor withdrew consent, which GDPR Art. 7(3) does not
  // allow. Both are called: opt out, then drop the identifiers already set.
