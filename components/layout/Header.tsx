        ...
        <nav className="hidden lg:flex items-center gap-9" aria-label="Main navigation">
          {navLinks.map(({ href, label }) => (
            <Link
              key={href}
              href={href}
              className={`relative font-mono text-[11px] tracking-[0.2em] uppercase pb-1 transition-colors duration-200 group ${
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="hidden lg:flex items-center border border-white/[0.12]">
            <Link
              href={pathname}
              locale="bg"
              className={`px-2.5 py-1.5 font-mono text-[9px] tracking-[0.15em] uppercase transition-colors duration-150 ${
                locale === 'bg' ? 'bg-amber text-navy-dk' : 'text-white/40 hover:text-white/70'
              }`}
            >
              BG
            </Link>
            <Link
              ...
              locale="en"
              className={`px-2.5 py-1.5 font-mono text-[9px] tracking-[0.15em] uppercase transition-colors duration-150 ${
                locale === 'en' ? 'bg-amber text-navy-dk' : 'text-white/40 hover:text-white/70'
              }`}
            >
              EN
            </Link>
          ...
          >
            <div className="relative w-5 h-[13px]">
              <span
                className={`absolute left-0 w-5 h-px bg-white transition-all duration-300 ${
                  mobileNavOpen ? 'top-[6px] rotate-45' : 'top-0'
                }`}
              />
              <span
                className={`absolute top-[6px] left-0 w-5 h-px bg-white transition-all duration-300 ${
                  mobileNavOpen ? 'opacity-0 scale-x-0' : ''
                }`}
              />
              <span
                className={`absolute h-px bg-white transition-all duration-300 ${
                  mobileNavOpen ? 'w-5 left-0 top-[6px] -rotate-45' : 'w-3 right-0 top-[12px]'
                }`}
              />
            </div>
          </button>
